...
$bd = new conexion();
	...
	$verificar = $bd->Consulta("select * from asistencia_refrigerio where mes=$mes and gestion=$gestion");
	...
	if($verificar_1[gestion]!=$gestion && $verificar_1[mes]!=$mes)
	{
		...
		while($registro_ac = $bd->getFila($registros_ac))
		{
			...
			$asistencia_refrigerio->registrar_asistencia_refrigerio($mes,$gestion,$dias_laborables,$dias_asistencia, $registro_ac[id_asignacion_cargo]);
