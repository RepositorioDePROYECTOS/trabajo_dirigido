<?php
  ...
  $precio_total = floatval(intval($cantidad_solicitada) * $precio_unitario);
  ...
  // if($precio_total > 50000) {
    if ($verificar_m[descripcion] != $descripcion && $verificar_m[unidad_medida] != $unidad_medida) {
      $registros = $bd->Consulta("INSERT INTO detalle_material (id_detalle_material, descripcion, unidad_medida, cantidad_solicitada, cantidad_despachada, precio_unitario, precio_total, id_solicitud_material)
          VALUES('',
                '$descripcion',
                '$unidad_medida',
                '$cantidad_solicitada',
      ...
      if ($bd->numFila_afectada() > 0) {
        ...
        // $message = "INSERT INTO detalle_material 
        // VALUES('',
        // 	  '$descripcion',
        // 	  '$unidad_medida',
        // 	  '$cantidad_solicitada',
  ...
  // } else {
  //   echo json_encode(array("success" => false, "message" => "Error debe ser mayor a 50.000,00 Bs."));
  // }
?>
