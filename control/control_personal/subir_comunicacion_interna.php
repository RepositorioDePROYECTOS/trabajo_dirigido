...
$file_input        = (!empty($_FILES['comunicacion_interna']['name'])) ? $_FILES['comunicacion_interna'] : '';
$fecha_solicitud   = date("Y-m-d H:i:s");
$target_dir = "../../files/comunicacion_interna" . DIRECTORY_SEPARATOR;
$target_file = $target_dir . $id_trabajador . "_" . "COMUNICACION INTERNA" . "_" . $fecha_solicitud . ".pdf";
$uploadOk = 1;
...
if ($uploadOk == 0) {
    ...
    if (move_uploaded_file($file_input["tmp_name"], $target_file)) {
        $consulta = $bd->Consulta("INSERT INTO `files`(`id_trabajador`, `tipo`, `fecha_creacion`) VALUES ($id_trabajador, 'COMUNICACION INTERNA', '$fecha_solicitud')");
