...
if(!empty($registro_b))
{
	$verificar = $bd->Consulta("select * from bono_antiguedad where mes=$mes and gestion=$gestion and id_asignacion_cargo=$id_asignacion_cargo");
	$verificar_1 = $bd->getFila($verificar);
	if($verificar_1[gestion]!=$gestion && $verificar_1[mes]!=$mes && $verificar[id_asignacion_cargo] != $id_asignacion_cargo)
	{
		$registros_ac = $bd->Consulta("select * from asignacion_cargo ac inner join trabajador t on t.id_trabajador=ac.id_trabajador inner join asistencia a on a.id_asignacion_cargo=ac.id_asignacion_cargo where ac.estado_asignacion='HABILITADO' and a.mes=$mes and a.gestion=$gestion and a.id_asignacion_cargo=$id_asignacion_cargo ");
		...
		while($registro_ac = $bd->getFila($registros_ac))
		{
			...
			if($antiguedad[0] >= 2)
			{
				...
				while($registro_ba = $bd->getFila($registros_ba))
				{
					...
					{
						$monto = (((2122*3*$registro_ba[porcentaje])/100)/$registro_ac[dias_laborables])*$registro_ac[dias_asistencia];
	...
	{
		echo "Error. Ya se tiene generada el bono de antiguedad del trabajador de ese periodo";
	}
}
else
{
