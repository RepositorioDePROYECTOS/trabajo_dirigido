...
$ufv_actual = utf8_decode($_POST[ufv_actual]);
$ufv_pasado = utf8_decode($_POST[ufv_pasado]);

$impositiva = new impositiva();
$bd = new conexion();

$verificar_pls = $bd->Consulta("select * from total_ganado tg inner join asignacion_cargo ac on tg.id_asignacion_cargo=ac.id_asignacion_cargo inner join aporte_laboral al on al.id_asignacion_cargo=ac.id_asignacion_cargo inner join refrigerio r on r.id_asignacion_cargo=ac.id_asignacion_cargo where tg.mes=$mes and tg.gestion=$gestion");
if($ufv_actual<>$ufv_pasado)
{
	$verificar_pl = $bd->getFila($verificar_pls);
	if(!empty($verificar_pl))
	{
		$verificar = $bd->Consulta("select * from impositiva where mes=$mes and gestion=$gestion");
	...
	echo "Error. UFV pasado no debe ser mayor o igual al UFV actual";
?>