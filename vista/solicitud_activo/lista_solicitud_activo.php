...
if ($_SESSION[nivel] == 'PRESUPUESTO') {
    ...
    $registros = $bd->Consulta("SELECT * FROM solicitud_activo s 
        ...
        LEFT JOIN eventual e ON e.id_eventual = u.id_eventual
        LEFT JOIN derivaciones as d ON d.id_solicitud = s.id_solicitud_activo 
        WHERE s.estado_solicitud_activo='APROBADO'   
        and s.existencia_activo='NO' 
    ...
    $registros = $bd->Consulta("SELECT * FROM solicitud_activo s 
        ...
        LEFT JOIN derivaciones as d ON d.id_solicitud = s.id_solicitud_activo
        WHERE (
        s.estado_solicitud_activo='SOLICITADO' 
    ...
    //! Validamos que toda solictud que el gerente pueda ver, necesariamente haya pasadó por el respectivo paso de las compras menores
    $registros = $bd->Consulta("SELECT * FROM solicitud_activo s 
        INNER JOIN usuario u on u.id_usuario=s.id_usuario 
        LEFT JOIN trabajador t on t.id_trabajador=u.id_trabajador 
        LEFT JOIN eventual e ON e.id_eventual = u.id_eventual
        ...
        OR s.estado_solicitud_activo='VERIFICADO' 
        OR s.estado_solicitud_activo='PRESUPUESTADO' 
        OR s.estado_solicitud_activo='PROVEEDOR ASIGNADO' 
        OR s.estado_solicitud_activo='VISTO BUENO RPA' 
        OR s.estado_solicitud_activo='VISTO BUENO G.A.' 
        OR s.estado_solicitud_activo='SIN VISTO BUENO G.A.' 
        OR s.estado_solicitud_activo='SIN VISTO BUENO RPA'
        OR s.estado_solicitud_activo='MEMORANDUN' )
        AND s.programa_solicitud_activo IS NULL
        AND s.actividad_solicitud_activo IS NULL
        ORDER BY s.id_solicitud_activo desc");
    ...
    $registros = $bd->Consulta("SELECT * FROM solicitud_activo s 
        ...
        LEFT JOIN eventual e ON e.id_eventual = u.id_eventual
        INNER JOIN derivaciones as d ON d.id_solicitud = s.id_solicitud_activo
        WHERE s.estado_solicitud_activo='PRESUPUESTADO' 
        AND s.programa_solicitud_activo IS NULL
        AND s.actividad_solicitud_activo IS NULL
        ORDER BY s.id_solicitud_activo desc");
}
