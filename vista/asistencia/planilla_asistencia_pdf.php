...
?>
<h1 align="center">PLANILLA DE ASISTENCIA</h1>
...
<?php
    ...
    while($registro_c = $bd->getFila($registros_c))
    {
        ...
        while($registro_ac = $bd->getFila($registros_ac))
        {
            ...
            {
                $registros = $bd->Consulta("select * from cargo c right join asignacion_cargo ac on c.id_cargo=ac.id_cargo inner join trabajador t on t.id_trabajador=ac.id_trabajador inner join asistencia a on a.id_asignacion_cargo=ac.id_asignacion_cargo where a.mes=$mes and a.gestion=$gestion and c.seccion='$registro_c[0]' and c.item=$registro_ac[item] order by cast(ac.item as unsigned) asc");
    ...
    try
    {
        ...
        $html2pdf->Output('planilla_asistencia.pdf');
    }
