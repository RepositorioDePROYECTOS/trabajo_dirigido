<?php
    ...
    $id_planilla = $_SESSION[id];
    require_once('../reportes/cabeza_logo_papeleta.php');
    include("../../modelo/planilla.php");
    include("../reportes/inicio_pagina_logo.php");
    $planilla = new planilla();
    $planilla->get_planilla($id_planilla);
    $mes = $planilla->mes;
    $gestion = $planilla->gestion;
    $nombre_mes = getMes($mes);
    $registros = $bd->Consulta("select * from planilla p inner join cargo c on p.item=c.item inner join asignacion_cargo ac on ac.id_cargo=c.id_cargo inner join bono_antiguedad ba on ba.id_asignacion_cargo=ac.id_asignacion_cargo where p.id_planilla=$id_planilla and ba.mes=$mes and ba.gestion=$gestion and p.estado_planilla='APROBADO'");
...
<table align="center" width="600" class="tabla_reporte">
    ...
    <tr>
        <td><strong>NOMBRE:</strong></td><td><?php echo utf8_encode($registro[nombres])." ".utf8_encode($registro[apellidos]);?></td><td><strong>Nro. ASEGURADO:</strong></td><td><?php echo $registro[nua];?></td>
    </tr>
    <tr>
...
{
    echo "La planilla no esta aprobada para imprimir la boleta de pago";
}
    echo "</page>";
