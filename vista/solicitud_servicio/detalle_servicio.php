...
$registros_s = $bd->Consulta("SELECT  * 
    ...
    LEFT JOIN requisitos as r ON r.id_detalle = d.id_detalle_servicio
    LEFT JOIN proveedores as pr ON pr.id_proveedor = r.id_proveedor
    WHERE d.id_solicitud_servicio=$id_solicitud_servicio");
    ...
    <div class="panel-body">
        <div class="row">
            <div class="col-sm-12">
                ...
                <?php
                $objetivo = $bd->Consulta("SELECT objetivo_contratacion FROM solicitud_servicio WHERE id_solicitud_servicio = $id_solicitud_servicio");
                ...
                <?php
                $justificativo = $bd->Consulta("SELECT justificativo FROM solicitud_servicio WHERE id_solicitud_servicio = $id_solicitud_servicio");
            ...
            <?php if ($_SESSION[nivel] == 'PRESUPUESTO') { ?>
                <?php if ($registro_sol[estado_solicitud_servicio] == 'VERIFICADO') { ?>
                    <div class="col-sm-4">
                        ...
                        <tbody id="tbl_body">
                            ...
                            while ($registro_s = $bd->getFila($registros_s)) {
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <input type="hidden" name="id_detalle_<?php echo $n; ?>" id="id_detalle_<?php echo $n; ?>" value="<?php echo $registro_s[id_detalle_servicio]; ?>">
                                        <?php
                ...
                <div id="detalle_servicioes" class="col-sm-12 table-responsive">
                    <table class="table table-bordered table-striped">
                        ...
                        <tbody id="tbl_body">
                            ...
                            ?>
                            <tr>
                                <td colspan='7' align='right'>TOTAL: <?php echo numeroALetras($totales) ?></td>
                                <td><?php echo number_format($totales) ?></td>
                            </tr>
                        </tbody>
                    </table>
        ...
        <div class="form-group">
            ...
            <?php if ($_SESSION[nivel] == 'GERENTE ADMINISTRATIVO') { ?>
                <div class="col-sm-12">
                    <?php if ($registro_sol[estado_solicitud_servicio] == 'SOLICITADO') { ?>
                        <a href='control/solicitud_servicio/aprobar_solicitud.php?id=<?= $id_solicitud_servicio; ?>' class='accion btn btn-green btn-icon' style='float: right; margin-left: 5px;'>Aprobar <i class='entypo-pencil'></i></a>
                    ...
                    <?php } ?>
                    <?php if ($registro_sol[estado_solicitud_servicio] == 'VISTO BUENO RPA') { ?>
                        <a href="control/solicitud_servicio/aprobar_solicitud_ga.php?id=<?php echo $id_solicitud_servicio; ?>&estado=visto bueno G.A.&id_usuario=<?php echo $_SESSION[id_usuario]; ?>&tipo=servicio" class='accion btn btn-green btn-icon' style='float: right; margin-left: 5px;'>Visto Bueno <i class='entypo-pencil'></i></a>
                        <a href="control/solicitud_servicio/aprobar_solicitud_ga.php?id=<?php echo $id_solicitud_servicio; ?>&estado=sin visto bueno G.A.&id_usuario=<?php echo $_SESSION[id_usuario]; ?>&tipo=servicio" class='accion btn btn-red btn-icon' style='float: right; margin-left: 5px;'>
            ...
            <?php if ($_SESSION[nivel] == 'PRESUPUESTO') { ?>
                <div class="col-sm-12">
                    <?php if ($registro_sol[estado_solicitud_servicio] == 'VERIFICADO') { ?>
                        <?php 
                            $consultar = $bd->Consulta("SELECT observaciones FROM derivaciones WHERE tipo_solicitud='servicio' AND id_solicitud=$id_solicitud_servicio");
                            $daos_consulta = $bd->getFila($consultar);
                            if($daos_consulta[observaciones] != "SIN EXISTENCIA"){
                        ?>
                        <a href="documents/<?php echo $id_solicitud_servicio; ?>_servicio.pdf" target="_blank" class="btn btn-red pull-left">Verificacion de Inexistencia</a>
                        <?php } ?>
                        <button type="button" class="btn btn-default pull-right btn-icon" data-dismiss="modal">Cerrar <i class='entypo-cancel'></i></button>
                        <button id_solicitud=<? echo $id_solicitud_servicio; ?> id_usuario=<?php echo $_SESSION[id_usuario] ?> type="button" class="btn btn-success pull-right btn-icon ml-2" id="enviarPresupuesto" data-dismiss="modal">Enviar <i class='entypo-cancel'></i></button>
                    <?php } ?>
                </div>
            <?php } ?>
            ...
            <?php if ($_SESSION[nivel] == 'ADQUISICION') { ?>
                <div class="col-sm-12">
                    <?php if ($registro_sol[estado_solicitud_servicio] == 'APROBADO') { ?>
                        <!-- <a href='control/solicitud_servicio/solicitud_adquisicion.php?id=<?= $id_solicitud_servicio; ?>&estado=ADQUISICION'
                    ...
                    <?php } elseif ($registro_sol[estado_solicitud_servicio] == 'ADQUISICION') { ?>
                        <!-- <a href='control/solicitud_servicio/solicitud_adquisicion.php?id=<?= $id_solicitud_servicio; ?>&estado=COMPRADO'
            ...
            <?php if ($_SESSION[nivel] == 'RPA') { ?>
                <div class="col-sm-12">
                    <?php if ($registro_sol[estado_solicitud_servicio] == 'PRESUPUESTADO') { ?>
                        <a href="documents/presupuesto_<?php echo $id_solicitud_servicio; ?>_servicio.pdf" target="_blank" class="btn btn-red pull-left">Cerificación Presupuestaria</a>
                        <button id_solicitud="<?php echo $id_solicitud_servicio; ?>" id_usuario="<?php echo  $_SESSION[id_usuario]; ?>" totales="<?php echo $totales; ?> " class='btn btn-green btn-icon' style='float: right; margin-left: 5px;' id="aprobarRPA">
    ...
    <div class="modal-dialog modal-lg" style="margin-top:100px">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div class="row">
                    <form name="frm_solicitud_servicio" id="frm_solicitud_servicio" action="control/solicitud_servicio/rechazar_solicitud.php" method="post" role="form" class="validate_edit form-horizontal form-groups-bordered">
...
<script type="text/javascript">
    jQuery(document).ready(function($) {
        ...
        $('#enviarPresupuesto').click(function(e) {
            ...
            if (file) {
                $.ajax({
                    url: 'control/solicitud_servicio/presupuestar.php', // Ruta al controlador PHP
                    type: 'POST',
                    data: form_data,
                    processData: false,
                    contentType: false,
        ...
        $('#aprobarRPA').click(function(e) {
            ...
            jConfirm("¿Est&aacute; seguro de realizar la acci&oacute;n?. " + mensaje, "Mensaje", function(resp) {
                if (resp) {
                    ...
                    $.ajax({
                        url: 'control/solicitud_servicio/aprobar_solicitud.php', // Ruta al controlador PHP
                        type: 'POST',
                        data: {
                            'id': id,
                            'id_usuario': id_usuario,
