...
if($registro_t[item]){
	// echo "Hay";
	$registros_t = $bd->Consulta("SELECT * from asignacion_cargo ac inner join trabajador t on ac.id_trabajador=t.id_trabajador inner join usuario u on t.id_trabajador=u.id_trabajador inner join cargo c on c.id_cargo=ac.id_cargo where estado_asignacion='HABILITADO' and id_usuario=$id_usuario");
	while ($cargar = $bd->getFila($registros_t)) {
		// echo "Entro";
		// print_r($cargar);
		$seccion = utf8_encode($cargar[seccion]);
		$nombre_solicitante = utf8_encode($cargar[nombres] . " " . $cargar[apellido_paterno] . " " . $cargar[apellido_materno]);
		$item_solicitante = $cargar[item];
		$cargo = utf8_encode($cargar[cargo]);
	}
} else {
	// echo "no Hay";
...
<div class="panel panel-default panel-shadow" data-collapsed="0">
	...
	<div class="panel-body">
		<form name="frm_solicitud_material" id="frm_solicitud_material" action="control/solicitud_material/insertar_solicitud_material.php" method="post" role="form" class="validate form-horizontal form-groups-bordered">
			...
			<div class="form-group">
				...
				<div class="col-sm-2">
					<input type="text" name="fecha_solicitud" id="fecha_solicitud" class="form-control required datepicker" value="<?php echo $fecha; ?>" readonly />
				</div>
				...
				<div class="col-sm-2">
					<select name="existencia_material" id="existencia_material" class="form-control required select2">
						<option value="SELECCIONE">___Seleccione___</option>
			...
			<div class="form-group">
				<div class="col-sm-offset-3 col-sm-5">
					<button type="submit" class="btn btn-info" id="registrar" disabled>Registrar</button> <button type="reset" class="btn btn-default cancelar">Cancelar</button>
				</div>
			</div>
		</form>
	</div>
...
<script type="text/javascript">
	jQuery(document).ready(function($) {
		$('#existencia_material').change(function() {
			var valor = $(this).val();
			if (valor == 'SELECCIONE') {
