...
if ($_SESSION[nivel] == 'PRESUPUESTO') {
    ...
    $registros = $bd->Consulta("SELECT * FROM solicitud_material s 
        ...
        and s.existencia_material='NO' 
        order by s.id_solicitud_material DESC");
} elseif ($_SESSION[nivel] == 'GERENTE GENERAL') { // con visto bueno o sin visto bueno
    $registros = $bd->Consulta("SELECT * FROM solicitud_material s 
        INNER JOIN usuario u on u.id_usuario=s.id_usuario 
        LEFT JOIN trabajador t on t.id_trabajador=u.id_trabajador 
        LEFT JOIN eventual e ON e.id_eventual = u.id_eventual
        LEFT JOIN derivaciones as d ON d.id_solicitud = s.id_solicitud_material
        where ( s.estado_solicitud_material='SOLICITADO' 
        OR s.estado_solicitud_material='VERIFICADO' 
        OR s.estado_solicitud_material='PRESUPUESTADO' )
        AND s.programa_solicitud_material IS NULL
        AND s.actividad_solicitud_material IS NULL
        order by s.id_solicitud_material DESC");
    ...
    $registros = $bd->Consulta("SELECT * FROM solicitud_material s 
        ...
        order by s.id_solicitud_material DESC");
} elseif ($_SESSION[nivel] == 'GERENTE COMERCIAL') { // gurquizu
    $registros = $bd->Consulta("SELECT * FROM solicitud_material s 
        INNER JOIN usuario u on u.id_usuario=s.id_usuario 
        LEFT JOIN trabajador t on t.id_trabajador=u.id_trabajador 
        LEFT JOIN eventual e ON e.id_eventual = u.id_eventual
    ...
    //! Validamos que toda solictud que el gerente pueda ver, necesariamente haya pasadó por el respectivo paso de las compras menores en vista de gerente administrativo
    $registros = $bd->Consulta("SELECT * FROM solicitud_material s 
        INNER JOIN usuario u on u.id_usuario=s.id_usuario 
        LEFT JOIN trabajador t on t.id_trabajador=u.id_trabajador 
        LEFT JOIN eventual e ON e.id_eventual = u.id_eventual
        ...
        OR s.estado_solicitud_material='VERIFICADO' 
        OR s.estado_solicitud_material='PRESUPUESTADO' 
        OR s.estado_solicitud_material='PROVEEDOR ASIGNADO' 
        OR s.estado_solicitud_material='VISTO BUENO RPA' 
        OR s.estado_solicitud_material='VISTO BUENO G.A.' 
        OR s.estado_solicitud_material='SIN VISTO BUENO G.A.' 
        OR s.estado_solicitud_material='SIN VISTO BUENO RPA'
        OR s.estado_solicitud_material='MEMORANDUN')
        AND s.programa_solicitud_material IS NULL
        AND s.actividad_solicitud_material IS NULL
        order by s.id_solicitud_material DESC");
} elseif ($_SESSION[nivel] == 'RPA') { // RCaguilar
    //! Para no sobre cargar la vista, el RPA solo ve las solicitudes que cumplan en reglamento de compras menores
    $registros = $bd->Consulta("SELECT * FROM solicitud_material s 
        INNER JOIN usuario u on u.id_usuario=s.id_usuario 
        LEFT JOIN trabajador t on t.id_trabajador=u.id_trabajador 
        LEFT JOIN eventual e ON e.id_eventual = u.id_eventual
        INNER JOIN derivaciones as d ON d.id_solicitud = s.id_solicitud_material
        where s.estado_solicitud_material='PRESUPUESTADO' 
        AND s.programa_solicitud_material IS NULL
        AND s.actividad_solicitud_material IS NULL
        order by s.id_solicitud_material DESC");
}
...
//     where s.estado_solicitud_material='SOLICITADO' order by s.id_solicitud_material desc"); 
// }
?>
