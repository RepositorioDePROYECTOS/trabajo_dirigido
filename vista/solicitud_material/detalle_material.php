...
$validador = $bd->Consulta("SELECT existencia_material FROM solicitud_material WHERE id_solicitud_material = $id_solicitud_material");
$validacion = $bd->getFila($validador);
if ($validacion[existencia_material] == 'SI') {
    $registros_s = $bd->Consulta("SELECT * FROM detalle_material WHERE id_solicitud_material=$id_solicitud_material");
    // $mostrar = $bd->getFila($registros_s);
    // print_r($mostrar);
} else {
    $registros_s = $bd->Consulta("SELECT * 
        from detalle_material as d 
        LEFT JOIN partidas as p ON p.id_partida = d.id_partida 
        LEFT JOIN requisitos as r ON r.id_detalle = d.id_detalle_material
        LEFT JOIN proveedores as pr ON pr.id_proveedor = r.id_proveedor
        WHERE d.id_solicitud_material=$id_solicitud_material");
}
...
while ($registro_s = $bd->getFila($registros_s)) {
    $totales = $totales + ($registro_s[precio_unitario] * $registro_s[cantidad_despachada]);
}
?>
    ...
    <div class="panel-body">
        <div class="row">
            <div class="col-sm-12">
                ...
                <?php
                $objetivo = $bd->Consulta("SELECT objetivo_contratacion FROM solicitud_material WHERE id_solicitud_material = $id_solicitud_material");
                $data_objetivo = $bd->getFila($objetivo);
                ?>
                <input type="text" name="objetivo" id="objetivo" class="form-control" readonly value="<?php echo utf8_encode($data_objetivo[objetivo_contratacion]) ?>">
                <label for="justificativo" class="control-label">Justificativo</label>
                <?php
                $justificativo = $bd->Consulta("SELECT justificativo FROM solicitud_material WHERE id_solicitud_material = $id_solicitud_material");
                $data_justificativo = $bd->getFila($justificativo);
                ?>
                <input type="text" name="justificativo" id="justificativo" class="form-control" readonly value="<?php echo utf8_encode($data_justificativo[justificativo]) ?>">
            </div>
            <div class="col-sm-4">
            ...
            <?php if ($_SESSION[nivel] == 'PRESUPUESTO') { ?>
                <?php if ($registro_sol[estado_solicitud_material] == 'VERIFICADO') { ?>
                    <div class="col-sm-4">
            ...
            <?php } elseif (($_SESSION[nivel] == 'ALMACENERO' || $_SESSION[nivel] == 'PRESUPUESTO')  && $registro_sol[existencia_material] == 'NO' && ($registro_sol[estado_solicitud_material] == 'APROBADO' || $registro_sol[estado_solicitud_material] == 'SIN EXISTENCIA' || $registro_sol[estado_solicitud_material] == 'COMPRADO')) { ?>
                <div class="col-sm-4 bg-danger">
                    ...
                    <input type="text" name="existencia_material" id="existencia_material" class="form-control bg-danger" value='<?php echo $registro_sol[existencia_material]; ?>' disabled />
                </div>
                        ...
                        <tbody id="tbl_body">
                            ...
                            while ($registro_s = $bd->getFila($registros_s)) {
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <input type="hidden" name="id_detalle_<?php echo $n; ?>" id="id_detalle_<?php echo $n; ?>" value="<?php echo $registro_s[id_detalle_material]; ?>">
                                        <?php
                ...
                <div id="detalle_materiales" class="col-sm-12 table-responsive">
                    <table class="table table-bordered table-striped">
                        ...
                        <tbody id="tbl_body">
                            ...
                            <?php
                            // $registros_existencia = $bd->getFila($registros_s);
                            // if($registros_existencia){
                            mysql_data_seek($registros_s, 0);
                            // }
                            while ($registro_s = $bd->getFila($registros_s)) {
                                $n++;
                                ...
                                if ($_SESSION[nivel] == 'ALMACENERO' && $registro_sol[existencia_material] == 'SI' && $registro_sol[estado_solicitud_material] == 'APROBADO') {
                                    echo "<td><a href='?mod=solicitud_material&pag=editar_detalle_material&id_detalle_material=$registro_s[id_detalle_material]&id_solicitud_material=$id_solicitud_material' class='btn btn-info btn-icon view_modal_edit'>Editar cantidad <i class='entypo-pencil'></i></a></td>";
                                }
                                echo "</tr>";
                            }
                            ?>
        ...
        <div class="form-group">
            ...
            <?php if ($_SESSION[nivel] == 'GERENTE ADMINISTRATIVO') { ?>
                <div class="col-sm-12">
                    <?php if ($registro_sol[existencia_material] == 'SI') { ?>
                        <?php if ($registro_sol[estado_solicitud_material] == 'SOLICITADO') { ?>
                            <a href='control/solicitud_material/aprobar_solicitud.php?id=<?= $id_solicitud_material; ?>' class='accion btn btn-green btn-icon' style='float: right; margin-left: 5px;'>Aprobar <i class='entypo-pencil'></i></a>
                        ...
                        <?php } ?>
                    <?php } ?>
                    <?php if ($registro_sol[estado_solicitud_material] == 'VISTO BUENO RPA') { ?>
                        <a href="control/solicitud_material/aprobar_solicitud_ga.php?id=<?= $id_solicitud_material; ?>&estado=visto bueno G.A.&id_usuario=<?php echo $_SESSION[id_usuario]; ?>&tipo=material" class='accion btn btn-green btn-icon' style='float: right; margin-left: 5px;'>Visto Bueno <i class='entypo-pencil'></i></a>
                        <a href="control/solicitud_material/aprobar_solicitud_ga.php?id=<?= $id_solicitud_material; ?>&estado=sin visto bueno G.A.&id_usuario=<?php echo $_SESSION[id_usuario]; ?>&tipo=material" class='accion btn btn-red btn-icon' style='float: right; margin-left: 5px;'>
            ...
            <?php if ($_SESSION[nivel] == 'PRESUPUESTO') { ?>
                <div class="col-sm-12">
                    <?php if ($registro_sol[estado_solicitud_material] == 'VERIFICADO') { ?>
                        <?php 
                            $consultar = $bd->Consulta("SELECT observaciones FROM derivaciones WHERE tipo_solicitud='material' AND id_solicitud=$id_solicitud_material");
                            $daos_consulta = $bd->getFila($consultar);
                            if($daos_consulta[observaciones] != "SIN EXISTENCIA"){
                        ?>
                        <a href="documents/<?php echo $id_solicitud_material; ?>_material.pdf" target="_blank" class="btn btn-red pull-left">Verificacion de Inexistencia</a>
                        <?php } ?>
                        <button type="button" class="btn btn-default pull-right btn-icon" data-dismiss="modal">Cerrar <i class='entypo-cancel'></i></button>
                        <button id_solicitud=<? echo $id_solicitud_material; ?> id_usuario=<?php echo $_SESSION[id_usuario] ?> type="button" class="btn btn-success pull-right btn-icon ml-2" id="enviarPresupuesto" data-dismiss="modal">Enviar <i class='entypo-cancel'></i></button>
                    <?php } ?>
                </div>
            <?php } ?>
            ...
            <?php if ($_SESSION[nivel] == 'ADQUISICION') { ?>
                <div class="col-sm-12">
                    <?php if ($registro_sol[estado_solicitud_material] == 'APROBADO') { ?>
                        <a href='control/solicitud_material/solicitud_adquisicion.php?id=<?= $id_solicitud_material; ?>&estado=ADQUISICION' class='accion btn btn-green btn-icon' style='float: right; margin-left: 5px;'>En adquisición <i class='entypo-pencil'></i></a>
                    <?php } elseif ($registro_sol[estado_solicitud_material] == 'ADQUISICION') { ?>
                        <a href='control/solicitud_material/solicitud_adquisicion.php?id=<?= $id_solicitud_material; ?>&estado=COMPRADO' class='accion btn btn-blue btn-icon' style='float: right; margin-left: 5px;'>Comprado <i class='entypo-pencil'></i></a>
                    <?php } ?>
                    <button type="button" class="btn btn-default pull-right btn-icon" data-dismiss="modal">Cerrar <i class='entypo-cancel'></i></button>
                </div>
            <?php } ?>
            ...
            <?php if ($_SESSION[nivel] == 'RPA') { ?>
                <div class="col-sm-12">
                    <?php if ($registro_sol[estado_solicitud_material] == 'PRESUPUESTADO') { ?>
                        <a href="documents/presupuesto_<?php echo $id_solicitud_material; ?>_material.pdf" target="_blank" class="btn btn-red pull-left">Cerificación Presupuestaria</a>
                        <button id_solicitud="<?php echo $id_solicitud_material; ?>" id_usuario="<?php echo  $_SESSION[id_usuario]; ?>" totales="<?php echo $totales; ?> " class='btn btn-green btn-icon' style='float: right; margin-left: 5px;' id="aprobarRPA">
            ...
            <?php if ($_SESSION[nivel] == 'ALMACENERO' && $registro_sol[existencia_material] == 'SI' && $registro_sol[estado_solicitud_material] == 'APROBADO') { ?>
                <div class="col-sm-12">
                    <a href='control/solicitud_material/despachar_solicitud.php?id_solicitud_material=<?= $id_solicitud_material; ?>' class='btn btn-green btn-icon despachar_solicitud' style='float: right; margin-left: 5px;'>Despachar
            ...
            <?php } elseif ($_SESSION[nivel] == 'ALMACENERO' && $registro_sol[existencia_material] == 'NO' && $registro_sol[estado_solicitud_material] == 'APROBADO') { ?>
                <a href='control/solicitud_material/solicitud_sin_existencia.php?id=<?= $id_solicitud_material; ?>' class='accion btn btn-danger btn-icon' style='float: right; margin-left: 5px;'>Sin Existencia <i class='entypo-forward'></i></a>
                <button type="button" class="btn btn-default pull-right btn-icon" data-dismiss="modal">Cerrar <i class='entypo-cancel'></i></button>
            <?php }
            if ($_SESSION[nivel] == 'ALMACENERO' && $registro_sol[existencia_material] == 'NO' && $registro_sol[estado_solicitud_material] == 'COMPRADO') { ?>
                <a href='control/solicitud_material/solicitud_adquisicion.php?id=<?= $id_solicitud_material; ?>&estado=DESPACHADO SIN EXISTENCIA' class='accion btn btn-orange btn-icon' style='float: right; margin-left: 5px;'>Despachar material sin existencia <i class='entypo-pencil'></i></a>
                <button type="button" class="btn btn-default pull-right btn-icon" data-dismiss="modal">Cerrar <i class='entypo-cancel'></i></button>
            <?php } ?>
    ...
    <div class="modal-dialog modal-lg" style="margin-top:100px">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div class="row">
                    <form name="frm_solicitud_material" id="frm_solicitud_material" action="control/solicitud_material/rechazar_solicitud.php" method="post" role="form" class="validate_edit form-horizontal form-groups-bordered">
    ...
    jQuery(document).ready(function($) {
        ...
        $('#enviarPresupuesto').click(function(e) {
            ...
            for (let contador = 1; contador <= tamanio; contador++) {
                ...
                // let obj = [{
                //     id_detalle: select_detalles,
                //     id_partida: selectedValue
                // }];
                // array.push(obj); Agregar el valor al array
            }
            console.log("informacion de los detalles",array);

            // form_data.append('partidas[]', array);
            if (file) {
                $.ajax({
                    url: 'control/solicitud_material/presupuestar.php', // Ruta al controlador PHP
                    type: 'POST',
                    data: form_data,
                    processData: false,
                    contentType: false,
        ...
        $('#aprobarRPA').click(function(e) {
            ...
            jConfirm("¿Est&aacute; seguro de realizar la acci&oacute;n?. " + mensaje, "Mensaje", function(resp) {
                if (resp) {
                    ...
                    $.ajax({
                        url: 'control/solicitud_material/aprobar_solicitud.php', // Ruta al controlador PHP
                        type: 'POST',
                        data: {
                            'id': id,
                            'id_usuario': id_usuario,
