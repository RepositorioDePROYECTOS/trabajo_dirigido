  <?php include("modelo/bono_antiguedad.php");    
    $bono_antiguedad = new bono_antiguedad();
    $registros = $bd->Consulta("select * from bono_antiguedad ba inner join asignacion_cargo ac on ba.id_asignacion_cargo=ac.id_asignacion_cargo inner join trabajador t on t.id_trabajador=ac.id_trabajador where ac.estado_asignacion='HABILITADO' and ba.mes=");    
?>
<h2>BONOS DE ANTIGUEDAD
    <a href="?mod=bono_antiguedad&pag=form_bono_antiguedad" class="btn btn-green btn-icon" style="float: right;">
    	Crear bono individual<i class="entypo-plus"></i>
    </a>
    <a href="?mod=bono_antiguedad&pag=todos_bono_antiguedad" class="btn btn-info btn-icon" style="float: right; margin-right: 5px;">
