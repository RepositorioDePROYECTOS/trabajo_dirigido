    ...
    while($registro_c = $bd->getFila($registros_c))
    {
        ...
        while($registro_ac = $bd->getFila($registros_ac))
        {
            if($registro_ac[estado_asignacion] == 'HABILITADO')
            {
                ...
                echo "<tr><td>$registro_ac[item]</td><td>$registro_ac[descripcion]</td><td>$registro[apellido_paterno] $registro[apellido_materno] $registro[nombres]</td><td>$registro[dias_laborables]</td><td>$registro[dias_asistencia]</td><td>$registro[monto_refrigerio]</td><td>$registro[total_refrigerio]</td></tr>";
        ...
        echo "<tr><td colspan='6' align='center'>SECCION $registro_c[0]</td><td align='center'>".number_format($sum_d_s,2)."</td></tr>";
        
        $sum_d = $sum_d + $sum_d_s;
        
    }
    echo "<tr><td colspan='6' align='center'>TOTAL REFRIGERIO DEL MES</td><td align='center'>".number_format($sum_d,2)."</td></tr>";
    echo "</table>";
    echo "</page>";
