...
$solicituds = $bd->Consulta("SELECT * FROM detalle_activo WHERE id_detalle_activo=$id_detalle_activo");
$solicitud = $bd->getFila($solicituds);
// echo var_dump($solicitud);
$registros_solicitud = $bd->Consulta("SELECT s.fecha_solicitud, s.nombre_solicitante, s.existencia_activo, d.unidad_medida
FROM solicitud_activo as s 
INNER JOIN detalle_activo as d ON d.id_solicitud_activo = s.id_solicitud_activo 
WHERE d.id_detalle_activo=$id_detalle_activo GROUP BY d.id_detalle_activo");
...
<div class="panel panel-default panel-shadow" data-collapsed="0">
    ...
    <div class="panel-body">
        ...
        <!-- action="control/detalle_activo/update_detalle_activo.php" -->
        <form name="frm_update_detalle_activo" id="frm_update_detalle_activo" method="post" role="form" class="validate_venta form-horizontal form-groups-bordered">
            ...
            <div class="row" id="existe" style="display: none;">
                <div class="col-md-3">
                    ...
                    <select name="id_activo" id="id_activo" class=" id_activo form-control required select2">
                        ...
                        while ($registro_m = $bd->getFila($registros_m)) {
                            ...
                            echo utf8_encode("<option value='$registro_m[id_activo]' $selected_autorizado>$registro_m[codigo]-$registro_m[descripcion] ($registro_m[unidad_medida])</option>");
                        }
                        ?>
                ...
                <div class="col-md-3" id="key_cantidad">
                    <label for="cantidad_activo">Cantidad Existente</label>
                    <input type="text" name="cantidad_activo" id="cantidad_activo" class="form-control" value="" readonly />
                </div>
                <div class="col-md-2">
            ...
            <div id="no_existe" style="display: none;">
                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-6">
                        <textarea name="descripcion_activo" id="descripcion_activo" rows="4" class="form-control required text uppercase"><?php echo utf8_encode($solicitud[descripcion]); ?></textarea>
                    </div>
                </div>
                <!-- <div class="form-group">
...
<script type="text/javascript">
    window.addEventListener("load", function() {
        ...
        var id = document.getElementById('id_activo').value;
        console.log('ID_activo: ' + id);
        fetch('control/detalle_activo/detalle_activo.php?id_detalle_activo=' + id)
            .then(response => response.json())
            .then(data => {
                if (data.success === true && data.cantidad > 0) {
                    document.getElementById('cantidad_activo').value = data.cantidad + ' ' + data.unidad_medida + ' restantes';
                    document.getElementById('cant_mat').value = data.cantidad;
                } else {
                    document.getElementById('cantidad_activo').value = '';
    ...
    jQuery(document).ready(function($) {
        ...
        function agregar() {
            ...
            var cantidad_activo = $("#cantidad_activo").val();
            ...
            $.ajax({
                type: "POST",
                url: "control/detalle_activo/update_detalle_activo.php",
                data: {
